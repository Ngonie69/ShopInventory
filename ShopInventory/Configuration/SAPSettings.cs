...
public class SAPSettings
{
    ...
    // No AutoSyncEnabled / SyncIntervalHours / InitialDelayMinutes: the price catalogue is no longer
    // synced on a schedule, only from Web → Settings → Data Sync, so a leftover value in a deployed
    // config is ignored.
