...
public sealed class GetSalesInWarehousePostHandler(
    ISAPServiceLayerClient sapClient,
    IOptions<SAPSettings> settings,
    ILogger<GetSalesInWarehousePostHandler> logger
) : IRequestHandler<GetSalesInWarehousePostQuery, ErrorOr<WarehouseSalesResponseDto>>
{
    public async Task<ErrorOr<WarehouseSalesResponseDto>> Handle(
        GetSalesInWarehousePostQuery request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            logger.LogInformation("Retrieved {Count} sales items in warehouse {Warehouse} from {From} to {To} (POST)",
