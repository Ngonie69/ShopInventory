...
# The workbook this generator is written against. Named in the generated file so
# its provenance survives being run from a copy -- the sheet layout, the column
# pairs and the OVERRIDES below are all specific to this workbook, so a run
# against a differently-shaped one would not produce a usable catalogue anyway.
SOURCE_WORKBOOK = "REVISED  ROUTES 2026.xlsx"
...
ADDITIONS = {
    ...
    "MIDLANDS 2": [
        ...
        # Zvishavane is on both Midlands routes -- MIDLANDS 1 stops at TM
        # Zvishavane, MIDLANDS 2 at NR Zvishavane -- so the town cannot place it.
        # The two run on different days and their own stops invoice 90.9%
        # Wednesday and 95.1% Tuesday respectively; this shop invoices Tuesday
        # 5/5, which puts it on MIDLANDS 2. Small sample, so revisit if the
        # Zvishavane drop moves.
        "GAI080",   # Metropeech hypermarket Zvishavane
    ],
    "KARIBA": [
        # Chinhoyi is on both KARIBA (TM Chinhoyi South/North, NR Chinhoyi,
        # Megasave Chinhoyi) and PNP CENTRAL (Bhola Chinhoyi). Same tie-break:
        # KARIBA runs Tuesday and invoices 94.8% Tuesday, PNP CENTRAL runs
        # Wednesday; this shop invoices Tuesday 3/3. Small sample.
...
def emit(catalogue: list) -> str:
    lines = [
        "// <auto-generated>",
        "//     Generated by scripts/DeliveryRoutes/generate_delivery_routes.py from",
        f'//     "{SOURCE_WORKBOOK}", sheet "{SHEET}", with every business partner code',
...
def main() -> int:
    ...
    args.output.write_text(emit(catalogue), encoding="utf-8")
