...
<div class='footer'>
  <span class='confidential'>Confidential</span> &bull; {CompanyName} &bull; {SystemName} &bull; Generated {DateTime.Now:dd MMM yyyy HH:mm}
    ...
    private static int PodKpiStrip(IXLWorksheet ws, int row, int lastCol, params (string Label, string Value, XLColor? Color)[] kpis)
    {
        ...
        {
            var (startCol, endCol) = PodKpiColumns(metricIndex, kpis.Length, lastCol);
    ...
    /// <summary>The columns one KPI of <paramref name="count"/> spans across the sheet's width.</summary>
    private static (int Start, int End) PodKpiColumns(int index, int count, int lastCol)
    {
        int startCol = (int)Math.Floor(index * lastCol / (double)count) + 1;
        int endCol = (int)Math.Floor((index + 1) * lastCol / (double)count);
    ...
    private static void PodDisclaimerRow(
        IXLWorksheet ws, int row, int lastCol, DateTime now, string source = "SAP Business One and POD upload records")
    {
        ws.Range(row, 1, row, lastCol).Merge();
        var cell = ws.Cell(row, 1);
        cell.Value = $"This document was auto-generated by the Shop Inventory System on {now:dd MMM yyyy 'at' HH:mm} CAT. Data sourced from {source}.";
    ...
    private static int WritePodCreditedEmptyRow(IXLWorksheet ws, int row, int lastCol) =>
        PodEmptyRow(ws, row, lastCol, PodNoCreditedInvoicesLabel);

    private static int PodEmptyRow(IXLWorksheet ws, int row, int lastCol, string message)
    {
    ...
    /// </summary>
    private static void PodNoteRow(IXLWorksheet ws, int row, int lastCol, string text)
    {
        ws.Range(row, 1, row, lastCol).Merge();
        var cell = ws.Cell(row, 1);
        cell.Value = text;
    ...
    /// </summary>
    /// <remarks>
    /// Drawn in the POD report's house style (title bar, KPI strip, striped grid, navy totals) so
    /// the two workbooks read as one family. Every KPI and totals row is stated per currency; see
    /// the per-sheet writers for how each keeps USD and ZWG apart.
    /// </remarks>
    ...
    private sealed record DesktopAnalysisContext(
    ...
    private static DesktopAnalysisMatrixRow DesktopAnalysisBreakdownRow(DesktopSalesBreakdownRow row) =>
        new(row.Key, null, row.Label, row.SalesCount, row.TotalAmount, row.ShareOfValuePercent, row.ByPaymentMethod);

    private static string DesktopAnalysisTenderName(string method) => method == "Ecocash" ? "EcoCash" : method;

    // Only a wallet is reconciled by its reference; for anything else the count is every sale and
    // would read as a problem that is not one.
    private static bool DesktopAnalysisIsWallet(string method) => method is "Ecocash" or "Innbucks";

    /// <summary>
    /// One line per currency, so a KPI never states a single figure summed across USD and ZWG.
    /// </summary>
    private static string DesktopAnalysisPerCurrency(
        IEnumerable<DesktopSalesCurrencyAnalysis> sections, Func<DesktopSalesCurrencyAnalysis, string?> value)
    {
        var lines = sections
            .Select(section => (section.Currency, Value: value(section)))
            .Where(entry => !string.IsNullOrWhiteSpace(entry.Value))
            .Select(entry => $"{entry.Currency} {entry.Value}")
            .ToList();
    ...
    /// <summary>
    /// The POD KPI strip, with counts written as numbers rather than text (no "number stored as
    /// text" flag) and the value row grown to hold one line per currency.
    /// </summary>
    private static int DesktopAnalysisKpiStrip(
        IXLWorksheet ws, int row, int lastCol, params (string Label, XLCellValue Value, XLColor? Color)[] kpis)
    {
        var next = PodKpiStrip(ws, row, lastCol, kpis
            .Select(kpi => (kpi.Label, kpi.Value.IsText ? kpi.Value.GetText() : kpi.Value.ToString(), kpi.Color))
            .ToArray());

        var lines = 1;
        for (var index = 0; index < kpis.Length; index++)
        {
            var cell = ws.Cell(row, PodKpiColumns(index, kpis.Length, lastCol).Start);
            var value = kpis[index].Value;

            if (value.IsNumber)
            {
                cell.Value = value;
                cell.Style.NumberFormat.Format = value.GetNumber() % 1 == 0 ? FormatCount : FormatQuantity;
            }
            else
            {
                var valueLines = value.GetText().Split('\n').Length;
                lines = Math.Max(lines, valueLines);
                cell.Style.Alignment.WrapText = valueLines > 1;
            }
        }

        if (lines > 1)
        {
            ws.Row(row).Height = 8 + 17 * lines;
            for (var index = 0; index < kpis.Length; index++)
                ws.Cell(row, PodKpiColumns(index, kpis.Length, lastCol).Start).Style.Font.FontSize = 12;
        }

        return next;
    }

    private static void DesktopAnalysisCurrencyCell(IXLCell cell, string currency)
    {
        cell.Value = currency;
        cell.Style.Font.Bold = true;
        cell.Style.Font.FontColor = PodTextMuted;
    ...
    private static void DesktopAnalysisCountCell(IXLCell cell, int count)
    {
        cell.Value = count;
        cell.Style.NumberFormat.Format = FormatCount;
        cell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
    }

    private static void DesktopAnalysisMoneyCell(IXLCell cell, decimal amount)
    {
        cell.Value = amount;
        StylePodCurrencyCell(cell, fontColor: amount == 0 ? PodTextMuted : null);
        cell.Style.NumberFormat.Format = FormatMoneyDashZero;
    }

    private static void DesktopAnalysisTotalCell(IXLCell cell, decimal amount, bool isStripe)
    {
        cell.Value = amount;
        StylePodTotalCell(cell, isStripe);
        cell.Style.NumberFormat.Format = FormatMoney;
    }

    private static void DesktopAnalysisShareCell(IXLCell cell, decimal percent, bool muted = false)
    {
        cell.Value = percent / 100m;
        cell.Style.NumberFormat.Format = FormatPercent;
        cell.Style.Font.FontColor = muted ? PodTextMuted : PodTextDark;
        cell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
    }

    private static void DesktopAnalysisSummaryNumber(IXLCell cell, XLCellValue value, string format)
    {
        cell.Value = value;
        cell.Style.NumberFormat.Format = format;
        cell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
    ...
    /// <summary>
    /// A navy totals row for one currency. Stacked for a multi-currency sheet, with a hairline
    /// between them so the USD and ZWG totals do not run together into one band.
    /// </summary>
    private static void DesktopAnalysisSummaryRow(IXLWorksheet ws, int row, int lastCol, string currency, string label, bool isFollowOn)
    {
        PodSummaryRow(ws, row, lastCol);
        if (isFollowOn)
        {
            // Both edges: the row above carries PodSummaryRow's medium navy bottom, which Excel
            // draws over this row's top.
            ws.Range(row - 1, 1, row - 1, lastCol).Style.Border.BottomBorder = XLBorderStyleValues.Thin;
            ws.Range(row - 1, 1, row - 1, lastCol).Style.Border.BottomBorderColor = XLColor.FromHtml("#4A7DAA");
            ws.Range(row, 1, row, lastCol).Style.Border.TopBorder = XLBorderStyleValues.Thin;
            ws.Range(row, 1, row, lastCol).Style.Border.TopBorderColor = XLColor.FromHtml("#4A7DAA");
        }
    ...
    /// <remarks>
    /// The title and disclaimer sit in merged cells, which clip rather than overflow. A narrow sheet
    /// (By Hour, or a matrix over a single tender) widens its label column to hold the title and
    /// wraps the disclaimer onto a second line.
    /// </remarks>
    private static void DesktopAnalysisFinish(
        ...
        int headerRow, int freezeCol, int lastDataRow, params double[] widths)
    {
        ...
        var mergedTitleWidth = widths.Take(lastCol - 1).Sum();
        if (widths.Length > 1 && mergedTitleWidth < titleWidth)
            widths[1] += titleWidth - mergedTitleWidth;

        // Likewise each KPI value (row 2): a KPI over a single narrow column widens that column.
        foreach (var cell in ws.Row(2).CellsUsed())
        {
            var span = ws.MergedRanges.FirstOrDefault(range => range.Contains(cell));
            var firstCol = cell.Address.ColumnNumber;
            var lastSpanCol = span?.RangeAddress.LastAddress.ColumnNumber ?? firstCol;
            var longestLine = cell.GetFormattedString().Split('\n').Max(line => line.Length);
            var needed = longestLine * (cell.Style.Font.FontSize >= 14 ? 1.45 : 1.25) + 2;
            var available = Enumerable.Range(firstCol, lastSpanCol - firstCol + 1)
                .Where(col => col <= widths.Length)
                .Sum(col => widths[col - 1]);
            if (lastSpanCol <= widths.Length && available < needed)
                widths[lastSpanCol - 1] += needed - available;
        }

        for (var index = 0; index < widths.Length; index++)
            ws.Column(index + 1).Width = widths[index];

        if (widths.Sum() < ws.Cell(disclaimerRow, 1).GetString().Length * 0.8)
        {
            ws.Cell(disclaimerRow, 1).Style.Alignment.WrapText = true;
            ws.Row(disclaimerRow).Height = 24;
        }
    }

    /// <remarks>
    /// Each currency is its own block — section title, table, totals row — because the totals and
    /// shares are the point of this sheet and neither may be taken across USD and ZWG.
    /// </remarks>
    private static void WriteDesktopAnalysisPaymentMethods(XLWorkbook workbook, DesktopAnalysisContext context)
    {
        ...
        {
            PodSectionTitle(ws, row, lastCol,
                $"{section.Currency} · {FormatPodCount(section.SalesCount, "sale", "sales")} over "
                + $"{FormatPodCount(section.DaysTraded, "day", "days")} · {context.Scope}");
            row = PodColumnHeaders(ws, row + 1, lastCol, headers);
            ...
            foreach (var method in section.ByPaymentMethod)
            {
                ...
                ws.Cell(row, 2).Style.Font.Bold = true;
                DesktopAnalysisCountCell(ws.Cell(row, 3), method.SalesCount);
                DesktopAnalysisShareCell(ws.Cell(row, 4), method.ShareOfCountPercent, muted: true);
            ...
            if (section.ByPaymentMethod.Count == 0)
                row = PodEmptyRow(ws, row, lastCol, "No sales.");

            DesktopAnalysisSummaryRow(ws, row, lastCol, section.Currency, "TOTAL", isFollowOn: false);
            ws.Cell(row, 3).Value = section.SalesCount;
            ws.Cell(row, 3).Style.NumberFormat.Format = FormatCount;
            ws.Cell(row, 3).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            DesktopAnalysisSummaryNumber(ws.Cell(row, 4), 1, FormatPercent);
    ...
    /// <remarks>
    /// One table with a Currency column rather than a sheet per currency: tills sell in USD and ZWG
    /// and one filter separates them. The totals are a navy row per currency beneath the table,
    /// outside the filter, and no total is ever taken across that column.
    /// </remarks>
    private static void WriteDesktopAnalysisMatrix(
        XLWorkbook workbook, DesktopAnalysisContext context, DesktopAnalysisMatrixSheet sheet)
    {
        var report = context.Report;
        ...
        var row = PodTitleBar(ws, $"{sheet.Title} - {context.Period}", lastCol, context.GeneratedAt);
        row = DesktopAnalysisKpiStrip(ws, row, lastCol,
            (sheet.TopLabel, DesktopAnalysisPerCurrency(
                sections.Select(entry => entry.Section),
                section => sections.First(entry => entry.Section == section).Rows
                    .MaxBy(line => line.TotalAmount)?.Label), PodGreen),
            (sheet.CountLabel, sections
        ...
        PodSectionTitle(ws, row, lastCol, $"{sheet.SectionTitle} · {context.Scope}");
        var headerRow = row + 1;
        row = PodColumnHeaders(ws, headerRow, lastCol, [.. headers]);

        var rowIndex = 0;
        foreach (var (section, rows) in sections)
        {
            // The best line is only worth marking when the currency has more than one to beat.
            var top = rows.Count > 1 ? rows.MaxBy(line => line.TotalAmount) : null;

            for (var lineIndex = 0; lineIndex < rows.Count; lineIndex++)
            {
                var line = rows[lineIndex];
                var isStripe = rowIndex++ % 2 == 1;
                PodDataRow(ws, row, lastCol, isStripe);

                if (lineIndex == 0 && row > headerRow + 1)
                {
                    ws.Range(row, 1, row, lastCol).Style.Border.TopBorder = XLBorderStyleValues.Medium;
                    ws.Range(row, 1, row, lastCol).Style.Border.TopBorderColor = PodGridColor;
                }
                ...
                if (line.Date is { } date)
                {
                    ...
                    labelCell.Style.NumberFormat.Format = FormatDayDate;
                ...
                foreach (var method in methods)
                {
                    DesktopAnalysisMoneyCell(ws.Cell(row, col++),
                        line.Parts.FirstOrDefault(part => part.PaymentMethod == method)?.TotalAmount ?? 0m);
                }

                DesktopAnalysisTotalCell(ws.Cell(row, takingsCol), line.TotalAmount, isStripe);
                if (sheet.WithShare)
                    DesktopAnalysisShareCell(ws.Cell(row, takingsCol + 1), line.SharePercent);
        ...
        for (var index = 0; index < sections.Count; index++)
        {
            ...
            foreach (var method in methods)
            {
                DesktopAnalysisSummaryNumber(ws.Cell(row, col++),
                    section.ByPaymentMethod.FirstOrDefault(part => part.PaymentMethod == method)?.TotalAmount ?? 0m,
                    FormatMoneyDashZero);
            }

            DesktopAnalysisSummaryNumber(ws.Cell(row, takingsCol), section.TotalAmount, FormatMoney);
            if (sheet.WithShare)
                DesktopAnalysisSummaryNumber(ws.Cell(row, takingsCol + 1), 1, FormatPercent);
    ...
    private static void WriteDesktopAnalysisHours(XLWorkbook workbook, DesktopAnalysisContext context)
    {
        const int lastCol = 6;
        var report = context.Report;
        var ws = workbook.Worksheets.Add("By Hour");
        PodApplyDefaults(ws);
        ...
        row = DesktopAnalysisKpiStrip(ws, row, lastCol,
            ...
            ("Busiest Hour", DesktopAnalysisPerCurrency(report.Currencies,
                section => section.ByHour.MaxBy(hour => hour.TotalAmount) is { } peak ? HourLabel(peak) : null), PodGreen),
            ("Sales", report.Currencies.Sum(section => section.SalesCount), PodNavy),
            ("Takings", DesktopAnalysisPerCurrency(report.Currencies, section => section.TotalAmount.ToString("N2")), PodNavy));

        PodSectionTitle(ws, row, lastCol, $"Takings by the hour they were rung up, summed across the period · {context.Scope}");
        var headerRow = row + 1;
        row = PodColumnHeaders(ws, headerRow, lastCol,
            ["Currency", "Hour (CAT)", "Sales", "Average Sale", "Takings", "Share of Takings"]);

        var rowIndex = 0;
        foreach (var section in report.Currencies)
        {
            var peak = section.ByHour.Count > 1 ? section.ByHour.MaxBy(hour => hour.TotalAmount) : null;

            for (var lineIndex = 0; lineIndex < section.ByHour.Count; lineIndex++)
            {
                var hour = section.ByHour[lineIndex];
                var isStripe = rowIndex++ % 2 == 1;
                PodDataRow(ws, row, lastCol, isStripe);
                ...
                DesktopAnalysisCurrencyCell(ws.Cell(row, 1), section.Currency);
                ws.Cell(row, 2).Value = HourLabel(hour);
                ws.Cell(row, 2).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                if (ReferenceEquals(hour, peak))
                    StylePodStatusCell(ws.Cell(row, 2), true, isStripe);

                DesktopAnalysisCountCell(ws.Cell(row, 3), hour.SalesCount);
                DesktopAnalysisMoneyCell(ws.Cell(row, 4), hour.SalesCount > 0 ? Math.Round(hour.TotalAmount / hour.SalesCount, 2) : 0m);
                DesktopAnalysisTotalCell(ws.Cell(row, 5), hour.TotalAmount, isStripe);
                DesktopAnalysisShareCell(ws.Cell(row, 6),
                    section.TotalAmount != 0 ? Math.Round(hour.TotalAmount / section.TotalAmount * 100m, 1) : 0m);
        ...
        {
            var section = report.Currencies[index];
            DesktopAnalysisSummaryRow(ws, row, lastCol, section.Currency,
                $"TOTAL · {FormatPodCount(section.ByHour.Count, "hour", "hours")}", isFollowOn: index > 0);
            ws.Cell(row, 3).Value = section.SalesCount;
            ws.Cell(row, 3).Style.NumberFormat.Format = FormatCount;
            ws.Cell(row, 3).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            DesktopAnalysisSummaryNumber(ws.Cell(row, 4), section.AverageSale, FormatMoney);
            DesktopAnalysisSummaryNumber(ws.Cell(row, 5), section.TotalAmount, FormatMoney);
            DesktopAnalysisSummaryNumber(ws.Cell(row, 6), 1, FormatPercent);
            row++;
        }

        DesktopAnalysisFinish(ws, row + 1, lastCol, context, headerRow, 2, lastDataRow,
            12, 20, 12, 16, 18, 17);
    }

    private static void WriteDesktopAnalysisItems(XLWorkbook workbook, DesktopAnalysisContext context)
    {
        const int lastCol = 8;
        var report = context.Report;
        ...
        row = DesktopAnalysisKpiStrip(ws, row, lastCol,
            ...
            ("Top Seller", DesktopAnalysisPerCurrency(report.Currencies,
                section => section.TopItems.FirstOrDefault() is { } item
                    ? string.IsNullOrWhiteSpace(item.ItemDescription) ? item.ItemCode : $"{item.ItemCode} · {item.ItemDescription}"
                    : null), PodGreen),
            ("Quantity Sold", (double)report.Currencies.Sum(section => section.QuantitySold), PodNavy),
            ("Value Before VAT", DesktopAnalysisPerCurrency(report.Currencies, section => section.NetAmount.ToString("N2")), PodNavy));

        PodSectionTitle(ws, row, lastCol, $"Items ranked by value sold, before VAT · {context.Scope}");
        var headerRow = row + 1;
        row = PodColumnHeaders(ws, headerRow, lastCol,
            ["Currency", "Rank", "Item Code", "Description", "Quantity", "Sales", "Value Before VAT", "Share of Line Value"]);

        var rowIndex = 0;
        foreach (var section in report.Currencies)
        {
            for (var lineIndex = 0; lineIndex < section.TopItems.Count; lineIndex++)
            {
                var item = section.TopItems[lineIndex];
                var rank = lineIndex + 1;
                var isStripe = rowIndex++ % 2 == 1;
                PodDataRow(ws, row, lastCol, isStripe);
                ...
                var rankCell = ws.Cell(row, 2);
                rankCell.Value = rank;
                rankCell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                if (rank <= 3)
                {
                    rankCell.Style.Font.Bold = true;
                    rankCell.Style.Font.FontColor = PodNavy;
                    rankCell.Style.Fill.BackgroundColor = isStripe ? PodTotalBg : PodSubHeaderBg;
                }
                else
                {
                    rankCell.Style.Font.FontColor = PodTextMuted;
                }

                ws.Cell(row, 3).Value = item.ItemCode;
                ws.Cell(row, 3).Style.Font.Bold = true;
                ws.Cell(row, 4).Value = string.IsNullOrWhiteSpace(item.ItemDescription) ? "-" : item.ItemDescription;
                ws.Cell(row, 5).Value = item.Quantity;
                StylePodCurrencyCell(ws.Cell(row, 5));
                ws.Cell(row, 5).Style.NumberFormat.Format = FormatQuantity;
                DesktopAnalysisCountCell(ws.Cell(row, 6), item.SalesCount);
                DesktopAnalysisTotalCell(ws.Cell(row, 7), item.NetAmount, isStripe);
                DesktopAnalysisShareCell(ws.Cell(row, 8), item.ShareOfNetPercent);
        ...
        if (rowIndex == 0)
            row = PodEmptyRow(ws, row, lastCol, "No lines were recorded on sales in this period.");

        var lastDataRow = row - 1;
        for (var index = 0; index < report.Currencies.Count; index++)
        {
            var section = report.Currencies[index];
            DesktopAnalysisSummaryRow(ws, row, lastCol, section.Currency, "TOTAL", isFollowOn: index > 0);

            // Totals of the items listed, not of the currency: the list is the head of what sold, and a
            // sale holding two listed items is one sale, so the Sales column has no total at all.
            ws.Cell(row, 3).Value = $"Top {section.TopItems.Count:N0} of {FormatPodCount(section.DistinctItems, "item", "items")}";
            DesktopAnalysisSummaryNumber(ws.Cell(row, 5), section.TopItems.Sum(item => item.Quantity), FormatQuantity);
            DesktopAnalysisSummaryNumber(ws.Cell(row, 7), section.TopItems.Sum(item => item.NetAmount), FormatMoney);
            DesktopAnalysisSummaryNumber(ws.Cell(row, 8), section.TopItems.Sum(item => item.ShareOfNetPercent) / 100m, FormatPercent);
            row++;
        }

        PodNoteRow(ws, row + 1, lastCol,
            "Values are before VAT. VAT is charged per line at each item's own rate and stored only against the sale, "
                + "so an item's VAT-inclusive value is not stated. Shares are of the net value of every line sold in that currency.");
        DesktopAnalysisFinish(ws, row + 3, lastCol, context, headerRow, 3, lastDataRow,
            12, 8, 14, 40, 12, 10, 18, 16);
    }
