...
/// <summary>
/// Ensures the Quartz ADO.NET (AdoJobStore) tables exist in the Web app's Postgres database before
/// the scheduler starts. Runs the canonical Quartz 3.x PostgreSQL schema idempotently and seeds the
/// per-scheduler row locks. Executed at startup (not via EF migration) so it works whether or not
/// the Web and API apps share a database. NOTE: validate against the target Postgres in staging —
/// this must match the Quartz version's official tables_postgres.sql.
/// </summary>
